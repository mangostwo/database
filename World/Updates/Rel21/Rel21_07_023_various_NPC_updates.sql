...
BEGIN
    ...
    SET @cNewDescription = 'various NPC updates';

                        -- COMMENT is 150 Characters MAX
    SET @cNewComment = 'various NPC updates';
	...
	UPDATE `creature_template` SET `SpeedRun`='1.14286' WHERE  `Entry`=1936;

	-- Morin Cloudstalker move between waypoints
	UPDATE `creature_template` SET `MovementType`='2' WHERE  `Entry`=2988;UPDATE `creature` SET `MovementType`=2, `modelid`=1624 WHERE `guid`=26577;DELETE FROM `creature_movement_template` WHERE `entry`=2988;INSERT INTO `creature_movement_template` (entry, point, position_x, position_y, position_z, orientation, waittime, script_id) VALUES
