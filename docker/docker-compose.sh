...
##### Database server #####
###########################
docker_path="/opt/mangos2/database";
...
user_command "${repo_path}" "SOURCE ${docker_path}/Character/Setup/characterLoadDB.sql" "character2"
...
then
    ...
    do
        echo "Importing Character update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/Character/Updates/Rel22/${sql}" "character2";
    done
fi
# find "${repo_path}/Character/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "character2"' _ {} \;

##########################
##### World Database #####
##########################
root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdCreateDB.sql" "" || echo "Realm DB already exists";
user_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdLoadDB.sql" "mangos2"
...
then
    ...
    do
        ...
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/FullDB/${sql}" "mangos2";
    done
fi
# find "${repo_path}/World/Setup/FullDB" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos2"' _ {} \;
...
then
    ...
    do
        ...
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Updates/Rel21/${sql}" "mangos2";
    done
fi
...
then
    ...
    do
        echo "Importing World Update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Updates/Rel22/${sql}" "mangos2";
    done
fi
# find "${repo_path}/World/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos2"' _ {} \;
